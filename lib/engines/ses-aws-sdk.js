...
// Expose to the world
module.exports = SDKSESTransport;
 ...
 */
function SDKSESTransport(options){
    this.options = options || {};

    //Set defaults if necessary
    this.options.ServiceUrl = this.options.ServiceUrl || "https://email.us-east-1.amazonaws.com";

    //set up AWS SDK with settings for SES
    AWS.config.update(options.awssettings);
    ses = new AWS.SES();
 ...
 */
SDKSESTransport.prototype.sendMail = function(emailMessage, callback) {
...
SDKSESTransport.prototype.handleMessage = function(options, email, callback) {
    var params = {
      RawMessage: { // required
...
SDKSESTransport.prototype.handleMessage = function(email, callback) {
    var request,
        date = new Date(),
...
SDKSESTransport.prototype.errorHandler = function(callback, err) {
...
SDKSESTransport.prototype.responseHandler = function(response, callback) {
 ...
 * @param {Function} callback Callback function to run on end (binded)
 * @param {Object} response HTTP Response object
SDKSESTransport.prototype.responseHandler = function(callback, response) {
    var body = "", match;
...
SDKSESTransport.prototype.generateMessage = function(emailMessage, callback) {
    var email = "";
